...
## §11 五轮修正：右列五键 + 键下方抽出候选 + quick-pair 切换键（2026-09-22，issue #28 round-5，用户实测反馈 4 条）

### §11.1 右列 = ⌫ + ，。！？ 五键

- 右窄列从 4 键改 **5 键**：退格固定最上，下面 ，。！？ 四颗符号键，
  原 4 键的列高由 5 键分摊。横屏 rail 保持八键 2×4（格数是四轮定稿，
  没有余格），符号键仍是 ，。
- 符号键交互从「按住上拖 → 浮层盖在键上方」（四轮）改成
  **「按住上拖 → 该键下方纵向抽出候选条」**：形态对齐 T9/数字面板
  左列符号条（`NUM_PAD_SYMBOLS` 那条线的窄列符号格），不再遮书写区
  ——四轮弹层的核心抱怨。手势语义不变：点按=键面字符直上屏、跟手
  高亮、松手输入选中格、拖出条外松手=取消，一段式无二次点选。
- 候选组（自身字形不进列表——点按就是它；按常用度排列）：
...
- 键盘视口内永不裁切：候选条缺省贴键下沿向下展开；下方空间装不下
  自然高度时先压格子（flex 收缩到 min 高），压到下限仍装不下（列内
  最底的键，竖屏 ？）才整条翻到键上方并限高。`closePopup` 统一清 `kp-drawer` 变体与按键宽展开的
  宽度，不污染其他长按弹层。

### §11.2 底行「写」键 → 标准 quick-pair 切换键

- 底行第 4 格换成与其他键盘底行同款的中英切换键
  （`cnEnKey`/`#modeToggle`，round-4 的 `inkModeKey` 删除）：
  **短按 = quick-pair 翻转**，**长按 = 完整模式菜单**（锚仍是这颗键）。
  键面大字=当前模式简写（手），右下小字=配对目标简写（拼）。
- 手写接入 quick-pair 体系：非手写模式记「上次使用的键盘」
  （`feelime_last_kb_mode`）；首次进手写时若配对还是出厂默认
  （拼/En），迁成 **手写↔上次键盘**——切换键两个方向都立即可用
  （拼 ⇄ 手 ⇄ 拼）。用户定制过的对（≠出厂默认）一律不动，改对走
  设置页配对编辑或 §11.3 的菜单入口。
- strictReady 挡位沿用 `toggleChineseEnglish` 既有语义：配对目标没
  就绪（如 笔画）时 toast「该键盘还在准备中」，不发 selectMode。

### §11.3 模式菜单：快捷切换入口 + 细节自查

- 菜单尾部新增**快捷切换**行（弱色 + 上分隔线，行语法与模式行一致：
  对内两个简写在 `prep` 位，名称在后）：点按关闭菜单直达设置页配对
  编辑。长按菜单本来就是切键盘的地方，改配对从这里一跳可达，手写
  场景不必先找到工具栏设置。
- 自查结论（保持不变的部分）：当前模式高亮（`.current`）、未就绪
  置灰 `…`（`.preparing` 且不可点）、菜单锚定触发键 + outside-tap
  关菜单时对锚定键 suppress 点击，都在 round-4 通道上继续成立；
  手写竖屏的锚定键现在就是底行切换键（`#modeToggle` 兜底命中）。

### §11.4 空书写区提示三行（双语）

- 「在此手写 · 长按清空」改为三行（用户定稿）：**在此手写 /
  模型能力有限 / 避免连笔以提高识别率**（Write here / Model has
  limits / Avoid cursive strokes for better results）。首行动作主行
  稍大，后两行预期管理弱化。清空入口不变（右列 ⌫ / 候选条 × /
  书写区长按）。

### §11.5 验证

- mock 用例（round-5 组）：右列五键构成、抽出条形态（`kp-drawer`）
  与「键下方」定位断言、各键候选组、拖出取消、短拖仍点按；底行
  quick-pair 短按翻转 + 长按菜单、菜单快捷切换入口直达配对编辑、
  配对迁移（含「定制过的不动」）；三行提示中英。
- AVD 门禁 `~/tmp/hw28/round5_gate.py`：识别类断言走桥级注入，
  布局/抽出条真实手势驱动，横屏 rail 回归。
- preview 截图 `~/tmp/hw-round5-preview-*.png`（深浅 × 竖屏 +
  抽出条展开态），**发布前需确认**。
