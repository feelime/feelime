...
## 6. 诊断可观测性（排查交互链路问题的标准流程）

黑盒交互（桥调用、深链跳转、触摸链）排查时**先建观测、再复现**，不要
目测截图猜。诊断体系（`Diagnostics.kt`）由设置页开关控制：内存环形
缓冲 400 条、只随「导出诊断」出设备、不落盘不进备份、**只记事件层位
与结构化值（锚 id / 屏幕几何 / 计数），不采用户输入内容**。

**排查流程**：设置页打开诊断开关 → 复现问题 → 导出诊断 → 按链对账。
已埋的关键链（tag `route`，2026-09-25 深链七轮连环修后建立）：

| 打点 | 含义 |
|---|---|
| `openSetupPage page=…` | JS 桥入口到达（缺 = 注解/JS 侧断） |
| `openSetup startActivity page=…` | Kotlin 发出跳转（page 空 = 只开首页） |
| `SetupActivity onCreate/onNewIntent target=…` | Activity 到达（冷启/热启两条） |
| `target hit=… attempt=N` | 锚命中（attempt>0 = 页面慢于首次注入） |
| `target MISS=…` | 重试耗尽未命中 |

配合已有 tag：`ui`（insets/焦点/触摸到达，issue #13 v3）、`engine`
（降级链路）。ace 上应用级 logcat 被 ColorOS 过滤（GC/framework 在、
应用 tag 全无）——**别指望 logcat，一律走诊断导出**；紧急定罪可临时
文件打点（`files/xxx.log` + run-as 读），合入前必须删净。

**验收纪律**（连环修的教训）：多环节交互的「验证通过」必须证明**链路
归属**——终态（如设置页显示目标内容）可能来自 am start 直发/上轮残留/
CDP 僵尸读数，只有链路打点能区分。改时序/状态语义前列全交互场景
（冷启/热启/搜索/预览触发的 ready 重入），每轮修完跑一遍场景清单，
只回归报障场景必然把新引入的 bug 带进下一轮。
